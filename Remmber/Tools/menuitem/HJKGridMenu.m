...
CGFloat const kHJKGridMenuDefaultDuration = 0.5f;
CGFloat const kHJKGridMenuDefaultBlur = 0.3f;
CGFloat const kHJKGridMenuDefaultWidth = 280;
...
- (UIImage *)rn_screenshot:(UIViewController *)control {
    UIView *view = [[[UIApplication sharedApplication] windows] lastObject];
    UIGraphicsBeginImageContext(view.bounds.size);
    //NSLog(@"w:%f,h:%f",view.bounds.size.width,view.bounds.size.height);
    ...
    //image = [UIImage imageNamed:@"星空1.jpg"];
//    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *docDir = [paths objectAtIndex:0];
//    NSString *path = [docDir stringByAppendingFormat:@"photo.png"];
...
#pragma mark - HJKGridMenuItem

@implementation HJKGridMenuItem

+ (instancetype)emptyItem {
    static HJKGridMenuItem *emptyItem = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        emptyItem = [[HJKGridMenuItem alloc] initWithImage:nil title:nil action:nil];
    });
...
- (BOOL)isEqual:(id)object {
    if (![object isKindOfClass:[HJKGridMenuItem class]]) {
        return NO;
    }
...
#pragma mark - HJKGridMenu

@interface HJKGridMenu ()
...
static HJKGridMenu *rn_visibleGridMenu;

@implementation HJKGridMenu
...
- (instancetype)initWithItems:(NSArray *)items {
    if ((self = [super init])) {
        ...
        _cornerRadius = 8.f;
        _blurLevel = kHJKGridMenuDefaultBlur;
        _animationDuration = kHJKGridMenuDefaultDuration;
        ...
        _menuStyle = HJKGridMenuStyleGrid;
        _itemTextAlignment = NSTextAlignmentCenter;
        _menuView = [UIView new];
        _backgroundColor = [UIColor colorWithWhite:0 alpha:0.3];
        _bounces = YES;

        BOOL hasImages = [items filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(HJKGridMenuItem *item, NSDictionary *bindings) {
            return item.image != nil;
        }]].count > 0;
        _menuStyle = hasImages ? HJKGridMenuStyleGrid : HJKGridMenuStyleList;
        _items = [items copy];
...
- (instancetype)initWithImages:(NSArray *)images {
    ...
    for (UIImage *image in images) {
        HJKGridMenuItem *item = [[HJKGridMenuItem alloc] initWithImage:image];
...
- (instancetype)initWithTitles:(NSArray *)titles {
    ...
    for (NSString *title in titles) {
        HJKGridMenuItem *item = [[HJKGridMenuItem alloc] initWithTitle:title];
        [items addObject:item];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    id<HJKGridMenuDelegate> delegate = self.delegate;

    if (self.selectedItemView != nil) {
        HJKGridMenuItem *item = self.items[self.selectedItemView.itemIndex];
...
- (void)viewWillLayoutSubviews {
    ...
    if (self.menuStyle == HJKGridMenuStyleGrid) {
        [self layoutAsGrid];
    }
    else if (self.menuStyle == HJKGridMenuStyleList) {
        [self layoutAsList];
    }
...
- (void)setupItemViews {
    ...
    [self.items enumerateObjectsUsingBlock:^(HJKGridMenuItem *item, NSUInteger idx, BOOL *stop) {
...
- (void)createScreenshotAndLayoutWithScreenshotCompletion:(dispatch_block_t)screenshotCompletion {
    if (self.blurLevel > 0.f) {
        ...
        UIImage *screenshot = [self.parentViewController.view rn_screenshot:nil];
...
- (void)showAnimated:(BOOL)animated {
    ...
    [self createScreenshotAndLayoutWithScreenshotCompletion:^{
        if (animated) {
            ...
            CATransform3D undershootScale = CATransform3DScale(self.menuView.layer.transform, 0.9, 0.9, 1.0);
            ...
            NSMutableArray *keyTimes = [NSMutableArray arrayWithObject:@0.1f];
...
- (void)dismissAnimated:(BOOL)animated {
    ...
    if (animated) {
        ...
        CATransform3D startingScale = self.menuView.layer.transform;
        CATransform3D overshootScale = CATransform3DScale(self.menuView.layer.transform, 1.1, 1.1, 1.0);
        CATransform3D undershootScale = CATransform3DScale(self.menuView.layer.transform, 0.9, 0.9, 1.0);
        CATransform3D endingScale = CATransform3DScale(self.menuView.layer.transform, 0, 0, 0);
...
- (void)selectItemViewAtPoint:(CGPoint)point {
    ...
    HJKGridMenuItem *item = self.items[selectedItemView.itemIndex];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchesDidMove) {
        HJKGridMenu *menu = [HJKGridMenu visibleGridMenu];
